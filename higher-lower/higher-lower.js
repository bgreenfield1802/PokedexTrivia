...
function makeGuess(guess, side) {
    ...
    if (guess == getWinner()) {
        ...
        correct += 1
        streak += 1
        if (highestStreak == null || streak > highestStreak) { highestStreak = streak }
        ...
        streak = 0
        if (timer < 0.6) {
            points += Math.round((0.6 - timer) * -20)
        }
    ...
    if (fastestChoice == null || timer < fastestChoice) { fastestChoice = timer }
    answers.push(side)
